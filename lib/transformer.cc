...
void RotateTransformer::TransformCanvas::SetImage(uint8_t *rawdata, int length, bool isBGR)
{
...
void UArrangementTransformer::TransformCanvas::SetImage(uint8_t *rawdata, int length, bool isBGR)
{
  if (width() * height() * 3 != length)
    return;
  for (int x = 0; x < width(); x++)
