...
void RGBMatrix::SetImage(uint8_t *rawdata, int length, bool isBGR)
{
  active_->SetImage(rawdata, length, isBGR);
}
...
public:
  ...
  virtual void SetImage(uint8_t *rawdata, int length, bool isBGR)
  {
    ...
    for (int x = 0; x < width(); x++)
    {
      for (int y = 0; y < height(); y++)
      {
        ...
        if (isBGR)
        {
          SetPixel(x, y, rawdata[offset + 2], rawdata[offset + 1], rawdata[offset]);
        }
        else
        {
          SetPixel(x, y, rawdata[offset], rawdata[offset + 1], rawdata[offset + 2]);
        }
      }
    }
  }
    ...
    const CanvasTransformer &transformer)
{
  ...
  CanvasTransformer *non_const_transformer = const_cast<CanvasTransformer *>(&transformer);
...
void FrameCanvas::SetImage(uint8_t *rawdata, int length, bool isBGR)
{
  frame_->SetImage(rawdata, length, isBGR);
}
