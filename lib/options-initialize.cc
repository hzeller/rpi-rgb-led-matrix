...
void PrintMatrixFlags(FILE *out, const RGBMatrix::Options &d,
  ...
  fprintf(out,
          ...
          "(Default: %d).\n"
          "\t--led-multiplexing=<0..5> : Mux type: 0=direct; 1=strip; 2=checker; 3=spiral; 4=Z-strip; 5=coreman (Default: 0)\n"
...
bool RGBMatrix::Options::Validate(std::string *err_in) const {
  ...
  if (multiplexing < 0 || multiplexing > 5) {
    err->append("Multiplexing can only be one of 0 (normal), 1 (snake), 2 (checkered), 3 (spiral), 4 (Z-stripe), 5(coreman) \n");
    success = false;
  }
