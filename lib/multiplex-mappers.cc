  ...
  namespace internal
  {
    ...
    public:
      ...
      void MapSinglePanel(int x, int y, int *matrix_x, int *matrix_y) const
      {
        ...
        const int y_quarter = y / quarter_rows;
        const int x_quarter = x / quarter_cols;
    ...
    public:
      ...
      void MapSinglePanel(int x, int y, int *matrix_x, int *matrix_y) const
      {
        ...
        const int vblock_is_odd = (y / tile_height) % 2;
        const int hblock = x / tile_width;

        if (vblock_is_odd)
        {
          *matrix_x = (x % tile_width) + (2 * tile_width * hblock) + tile_width;
        }
        else
        {
          *matrix_x = -((x % tile_width) - tile_width + 1) + (2 * tile_width * hblock);
        }
        *matrix_y = (y % tile_height) + tile_height * (y / (tile_height * 2));
      }
    };

    class P5_8S_64x32 : public MultiplexMapperBase
    {
    public:
      P5_8S_64x32() : MultiplexMapperBase("P5_8S_64x32", 2) {}
      ...
      void MapSinglePanel(int x, int y, int *matrix_x, int *matrix_y) const
      {
        ...
        *matrix_x = (x / pixelbase) * pixelbase * 2;
        if (is_top_stripe)
          *matrix_x += pixelbase + (x % pixelbase);
        else
          *matrix_x += x % pixelbase;
        *matrix_y = ((y / (panel_rows_ / 2)) * (panel_rows_ / 4) + y % (panel_rows_ / 4));
      }
    };
